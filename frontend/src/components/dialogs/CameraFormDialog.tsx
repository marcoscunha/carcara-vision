...
export const CameraFormDialog: React.FC<CameraFormDialogProps> = ({
  ...
  const isLocalCamera = formData.camera_type === 'local' || formData.camera_type === 'usb'
  ...
  const renderEditForm = () => (
    <Stack spacing={2} sx={{ pt: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        <Chip
          label={isLocalCamera ? 'Local Camera' : 'Network Camera'}
          size="small"
          color={isLocalCamera ? 'default' : 'primary'}
          variant="outlined"
        />
      </Box>
      <TextField
      ...
      {isLocalCamera ? (
        <TextField
          ...
          disabled
          helperText="Resolved automatically from the connected USB device"
        />
      ) : (
        <TextField
          margin="dense"
            ...
            <TabPanel value={tabValue} index={0}>
              <Stack spacing={2}>
                ...
                <TextField
                  ...
                  fullWidth
                  value={formData.rtsp_url || ''}
                  onChange={(e) => setFormData({ ...formData, rtsp_url: e.target.value, camera_type: 'ip' })}
                  disabled={isLoading}
                  placeholder="rtsp://192.168.1.100:554/stream"
                />
                <FormControl fullWidth margin="dense">
